...
이 문서는 관절각과 tool0 pose를 잇는 4가지 계산을 다루며, 각 절은 **문제, 유도, 코드 대응, 주의점, 검증**의 순서로 구성됨.
...
- 위 3행은 선속도 $v_e$ [m/s], 아래 3행은 각속도 $\omega_e$ [rad/s]
- 전부 **base 프레임** 기준
...
**프레임 인덱스가 한 칸 밀림** : standard DH의 $\theta_i$는 **이전 프레임의 z축** 둘레 회전이므로 (곱의 맨 앞이 $R_z(\theta)$, [1.2\_링크 변환 행렬 유도](#12-링크-변환-행렬-유도)), 관절 $i$의 축은 프레임 $i$가 아니라 프레임 $i{-}1$에 존재함.
...
- **rotation vector 단독** (`TestRotationVector`) : IK 전체를 돌리기 전에 오차 표현부터 격리해 확인하는 단계로, 항등행렬에서 0이 나오는지와 미소 회전 $10^{-4}$ rad, $90°$ 회전이 맞는지 확인함
- **FK와 IK 왕복** (`TestIKRoundtrip`) : 무작위 $q_{\text{true}}$로 목표를 만들고 IK가 그 목표를 재현하는지 확인하며, 허용치는 위치 1 mm, 자세 $0.1°$임
