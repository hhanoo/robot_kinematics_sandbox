...
특정 로봇이나 플래너에 종속되지 않는 범용 로봇 기구학·궤적 생성 실험용 기준 프로젝트로, CAD 파일과 DH 파라미터만 주어진 상황에서 ROS description 작성부터 기구학 알고리즘 구현·검증까지의 전체 파이프라인을 다루는 구조. 현재 대상 로봇은 UR10e이며, FK / Jacobian / IK를 라이브러리에 의존하지 않고 numpy로 직접 구현해 원리를 학습하는 것이 목표.
...
**Quintic 관절 궤적**: 5차 다항 rest-to-rest 프로파일을 관절 속도·가속도 한계 기반으로 시간 파라미터화
...
│   │   └── test/                    # FK / Jacobian / IK pytest (19)
│   │
│   ├── robot_trajectory/            # 궤적 생성 코어 (ROS import 없음)
│   │   ├── robot_trajectory/
│   │   │   ├── joint_traj.py        # quintic 프로파일 + 시간 파라미터화
...
- **fk.py**
  - `fk_frames(q)`: DH 행렬 누적 곱으로 base부터 각 관절까지 중간 프레임 (7, 4, 4) 반환 — Jacobian 계산에 재사용
...
- **ik.py**
  ...
  - `solve_ik`: DLS 업데이트 `Δq = Jᵀ(JJᵀ + λ²I)⁻¹e` + 스텝 상한 반복
    - λ² 항 덕분에 특이점에서도 역행렬이 항상 존재해 발산하지 않는 구조
...
- **cartesian_traj.py**
  ...
  - `circle_pose_path`: 축·중심 기준 원호 경로 (자세 고정)
  - `cartesian_to_joint`: waypoint마다 직전 해를 시드로 IK를 풀어 관절 점프 없는 연속 경로 생성, 실패 시 waypoint 인덱스 보고

### robot_bringup

- **demo_sequence.py**
  - zero → home(quintic) → IK 목표 pose(quintic) → 직선 10 cm → 반지름 8 cm 원 한 바퀴 → home 복귀
  - 세그먼트 경계 중복 행을 제거하고 인덱스 범위(`Segment`)와 함께 dt 간격 관절 행렬 하나로 반환
  - Cartesian 구간 IK 실패 시 세그먼트명·waypoint 번호를 담은 RuntimeError로 즉시 중단
- **demo_player.py**
...
joint_state_publisher_gui 슬라이더로 각 관절을 움직여 DH 기반 URDF와 메쉬 정렬을 확인.

### 2. 단위 테스트

```bash
test-kinematics   # 19 cases: FK vs URDF 체인, Jacobian vs 수치미분, IK 왕복
test-trajectory   # 20 cases: quintic 경계조건/한계, 경로 기하, 관절 연속성
```
...
RViz에서 zero → home → IK 목표 → 직선 → 원 → home 순서의 시퀀스가 반복 재생되는 구조.
...
| `quintic_joint_trajectory(q0, qf)`    | robot_trajectory.joint_traj     | 한계 기반 quintic 궤적                 |
...
> `docker/config.sh`의 `XAUTHORITY_PATH`가 실제 `~/.Xauthority` 경로와 일치하는지 확인.
